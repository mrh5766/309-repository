...
void MyMesh::GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	*/
	for (uint i = 0; i < a_nSubdivisions; i++)
	{
		...
		vector3 point1(a_fRadius * cos(2 * PI * i / a_nSubdivisions), a_fRadius * sin(2 * PI * i / a_nSubdivisions), 0);
		vector3 point2(a_fRadius * cos(2 * PI * (i + (uint)1) / a_nSubdivisions), a_fRadius * sin(2 * PI * (i + (uint)1) / a_nSubdivisions), 0);
